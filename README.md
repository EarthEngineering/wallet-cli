...
```
git clone https://github.com/earthengineering/wallet-cli.git
```
...
## How wallet-cli connects to java-earth:

Wallet-cli connect to java-earth by grpc protocol.  
Java-earth nodes can be deployed locally or remotely.  
We can set the connected java-earth node IP in config.conf of wallet-cli.
...
_amount:The amount of frozen funds，the unit is drop.
The minimum value is **1000000 drop(1EARTH)**._
...
- The share calculation method is: **1** unit of share can be obtained for every **1EARTH** frozen.
- After unfreezing, previous vote will expire. You can avoid the invalidation of the vote by re-freezing and voting.

**Note:** The EARTH Network only records the status of your last vote, which means that each of your votes will cover all previous voting results.

For example：

```
freezeBalance 100000000 3 1 address  // Freeze 10 EARTH  and acquire 10 units of shares
...
Assuming freeze 1EARTH（1_000_000 DROP），3 days，bandwidth obtained = 1* 1_000_000 * 3 = 3_000_000.
...
Applying to become a witness account needs to consume **100_000EARTH**.
...
Transfer to a non-existent account with a minimum transfer amount of **1EARTH**.
...
assetIssue 123456 testAssetIssue00001 10000000000000000 1 100 2018-4-1 2018-4-30 1 just-test https://github.com/earthengineering/wallet-cli/  
...
(cost EARTH 1000 EARTH for assetIssue)  
(You can query the EARTH balance and other asset balances for any account )  
...
AssetIssue AssetName TotalSupply EARTHNum AssetNum Precision StartDate EndDate Description Url
...
EARTHNum,AssetNum: these two parameters determine the exchange rate between the issued token
and the minimum unit of EARTH (sun) when the token is issued.  
FreeNetLimitPerAccount: The maximum amount of bandwidth an account is allowed to use. Token
issuers can freeze EARTH to obtain bandwidth (TransferAssetContract only)  
...
Example:  
AssetIssue TestEARTH 100000 1 1 2 "2019-04-04 11:48:00" "2019-04-05" "just for test" www.test.com
...
id0: the serial number of the parameter. Every parameter of EARTH network has a
serial number. Go to "https://www.earth.engineering/#/sr/committee" to see the specifics  
Value0: the modified value
In the example, modification No.4 (modifying token issuance fee) costs 1000EARTH as follows:  
...
which can be found in EARTH's related documents.  
a Create a trading pair  
exchangeCreate first*token_id first_token_balance second_token_id second_token_balance  
...
The ID is the ID of the issued TRC10 token. If it is EARTH, the ID is "*", the amount must be greater
than 0, and less than 1,000,000,000,000,000.  
Example:  
exchangeCreate 1000001 10000 \_ 10000  
Create trading pairs with the IDs of 1000001 and EARTH, the amount is 10000 for both.
...
It is expected to acquire the 80 EARTH by exchanging 1000001 from the transaction pair ID of 1, and
...
fee_limit: Transaction allows for the most consumed EARTH  
...
once  
value: The amount of EARTH transferred to the contract account  
token_value: Number of EARTH10  
token_id: EARTH10 Id  
Example:  
...
.  
fee_limit: The most amount of EARTH allows for the consumption
token_value: Number of EARTH10  
...
a delegate resource  
The latter two parameters are optional parameters. If not set, the EARTH is frozen to obtain
...
frozen_balance: The amount of frozen EARTH, the unit is the smallest unit (sun), the
